	...
	if !isAuth {
		...
		for i, _ := range messages {
			if messages[i].Receiver == keyVal["username"] && messages[i].ReadDate == "-" {
				...
				addReadInfo(messages[i].ID, now)
			}
		}
		json.NewEncoder(w).Encode(messages)
		log("CHATFETCH", keyVal["username"] + " fetched all messages", now)
	}
}
func getMessage(w http.ResponseWriter, r *http.Request) {
	...
	if !isAuth {
		...
		if message.Receiver == keyVal["username"] && message.ReadDate == "-" {
			...
			addReadInfo(message.ID, now)
		}
		json.NewEncoder(w).Encode(message)
		log("CHATFETCH", keyVal["username"] + " fetched a message with its id", now)
	}
}
...
func getUserMessages(w http.ResponseWriter, r *http.Request){
	...
	if !isAuth {
		...
		json.NewEncoder(w).Encode(messages)
		log("CHATFETCH", keyVal["username"] + " fetched all chat with " + params["username"], now)
	}
}
func sendMessage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log("DBPREPERR", "Database preparing failed",now)
		return
	}
	...
	log("USERCREATE", keyVal["username"] + " sent a message to " + params["username"], now)
}
...
func updateMessage(w http.ResponseWriter, r *http.Request) {
	...
	if !isAuth {
		...
		log("CHATUPDATE", keyVal["username"] + " updated a message", now)
	}
}
func deleteMessage(w http.ResponseWriter, r *http.Request) {
	...
	if !isAuth {
		...
		log("CHATDELETE", params["username"] + " deleted a message", now)
	}
}
